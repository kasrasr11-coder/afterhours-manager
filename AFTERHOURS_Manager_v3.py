...
def target(update):
    """Return the user targeted by replying to their message.

    Telegram can represent replies in a few forms. Prefer the replied-to
    message sender, then fall back to sender_chat when applicable.
    """
    ...
    if user is not None:
        return user
    # Messages sent on behalf of a chat/channel don't have a user target.
    return None


def target_debug(update):
    m = update.effective_message
    replied = getattr(m, "reply_to_message", None) if m else None
    if not replied:
        return "none"
    u = getattr(replied, "from_user", None)
    return f"message_id={getattr(replied, 'message_id', '?')}, user_id={getattr(u, 'id', None)}, name={getattr(u, 'first_name', None)}"
...
async def ban(update, context):
    ...
    u=target(update)
    if not u: return await update.effective_message.reply_text("⚠️ پیام هدف پیدا نشد. لطفاً روی پیام خودِ کاربر Reply کن و دوباره «بن» بفرست.")
...
async def unban(update, context):
    if not await need_admin(update): return
    u=target(update)
    if not u: return await update.effective_message.reply_text("⚠️ پیام هدف پیدا نشد. لطفاً روی پیام خودِ کاربر Reply کن و دوباره «رفع بن» بفرست.")
    try:
...
async def kick(update, context):
    if not await need_admin(update): return
    u=target(update)
    if not u: return await update.effective_message.reply_text("⚠️ پیام هدف پیدا نشد. لطفاً روی پیام خودِ کاربر Reply کن و دوباره «اخراج» بفرست.")
...
async def mute(update, context, minutes=None):
    if not await need_admin(update): return
    u=target(update)
    if not u: return await update.effective_message.reply_text("⚠️ پیام هدف پیدا نشد. لطفاً روی پیام خودِ کاربر Reply کن و دوباره «سکوت» بفرست.")
...
async def unmute(update, context):
    if not await need_admin(update): return
    u=target(update)
    if not u: return await update.effective_message.reply_text("⚠️ پیام هدف پیدا نشد. لطفاً روی پیام خودِ کاربر Reply کن و دوباره «رفع سکوت» بفرست.")
    try:
...
async def warn(update, context):
    if not await need_admin(update): return
    u=target(update)
    if not u: return await update.effective_message.reply_text("⚠️ پیام هدف پیدا نشد. لطفاً روی پیام خودِ کاربر Reply کن و دوباره «اخطار» بفرست.")
...
async def router(update, context):
    ...
    if text in {"بن", "رفع بن", "اخراج", "سکوت", "رفع سکوت", "اخطار"} or text.startswith("سکوت زمان‌دار") or text.startswith("سکوت زمان دار"):
        logging.info("MOD COMMAND text=%r reply_target=%s", text, target_debug(update))
